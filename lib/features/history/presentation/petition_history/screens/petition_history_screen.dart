...
class _PetitionHistoryScreenState
  ...
  Map<String, List<AnalysisHistory>> _groupByDate(
    ...
    for (final item in sorted) {
      final local = item.analyzedAt.toLocal();
  ...
  Future<void> _pickDateFilter() async {
    ...
    if (picked != null && mounted) {
      ref.read(historyViewModelProvider.notifier).updateDate(picked);
    }
  }
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Lista ────────────────────────────────────────────────────────
          Expanded(child: _buildBody(context, state, vm, grouped)),
        ],
      ),
    );
  }
          ...
          for (final entry in grouped.entries) ...[
            ...
            for (final item in entry.value)
              HistoryCard(
                id: item.petitionId,
                name: item.fileName,
                createdAt: item.analyzedAt.toLocal(),
                type: 'Petição',
